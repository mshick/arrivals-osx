...
SqliteStore.prototype.connect = function (cb) {
  ...
  self._db = new sqlite.Database(self._path, function (err) {
    ...
    self._db.exec(
      ...
      function (err) {
        if (err) return cb(err)
        self._db.get(`SELECT COUNT (*) FROM ${self._tableName} WHERE lock = ""`, function (
          err,
          results
        ) {
          if (err) return cb(err)
  ...
  self._putCargo = new async.cargo(function (tasks, cb) {
    ...
    self._db.run(`BEGIN`, function () {
      tasks.forEach(function (task) {
        ...
        self._db.run(
          `INSERT OR REPLACE INTO ${self._tableName} (id, task, priority, lock) VALUES (?, ?, ?, ?)`,
          [task.taskId, task.serializedTask, task.priority, ``]
        )
      })
...
SqliteStore.prototype.getTask = function (taskId, cb) {
  ...
  self._afterWritten(function () {
    self._db.all(
      `SELECT * FROM ${self._tableName} WHERE id = ? AND lock = ?`,
      [taskId, ``],
      function (err, rows) {
        if (err) return cb(err)
...
SqliteStore.prototype.getLock = function (lockId, cb) {
  ...
  self._afterWritten(function () {
    self._db.all(
      `SELECT id, task FROM ${self._tableName} WHERE lock = ?`,
      [lockId || ``],
      function (err, rows) {
        if (err) return cb(err)
        var tasks = {}
...
SqliteStore.prototype.takeFirstN = function (num, cb) {
  ...
  self._afterWritten(function () {
    self._db.run(
      `UPDATE ${self._tableName} SET lock = ? WHERE id IN (SELECT id FROM ${self._tableName} WHERE lock = ? ORDER BY priority DESC, added ASC LIMIT ${num})`,
      [lockId, ``],
      function (err) {
        if (err) return cb(err)
...
SqliteStore.prototype.takeLastN = function (num, cb) {
  ...
  self._afterWritten(function () {
    self._db.run(
      `UPDATE ${self._tableName} SET lock = ? WHERE id IN (SELECT id FROM ${self._tableName} WHERE lock = ? ORDER BY priority DESC, added DESC LIMIT ${num})`,
      [lockId, ``],
      function (err) {
        if (err) return cb(err)
        cb(null, this.changes ? lockId : ``)
