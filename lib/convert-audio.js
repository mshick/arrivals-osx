...
const checkVideoStream = function (_, metadata) {
...
const embedCoverFile = function (outputAudio, coverFile, binPaths) {
  ...
  log.info('embedCoverFile', binPaths.mp4box, '-itags', `cover=${coverFile}`, outputAudio, cwd);
...
module.exports = function (source, destination, binPaths) {
  ...
  return new Promise((resolve, reject) => {
    ...
    command.on('end', () => {
      ...
      if (coverFileEmbedded && fs.existsSync(coverFileEmbedded)) {
