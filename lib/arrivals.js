...
const watched = async (watcher, dispatcher) => {
  try {
    ...
    log.info(
      "Building existing files database for %s...",
      dispatcher.watchPath
    );

    await dispatcher.init();

    log.info(
      "Database for %s built in %d seconds",
      dispatcher.watchPath,
      timer.elapsed() / MS
    );
    ...
    watcher.on("change", async (filepath, info) => {
      ...
      try {
        if (
          info.type === "file" &&
          (info.event === "modified" || info.event === "moved-in")
        ) {
...
const main = () => {
  ...
  const watchedPaths = watchPaths.map(watchPath => {
    ...
    return watched(watcher, dispatcher);
  });

  Promise.all(watchedPaths);
};
